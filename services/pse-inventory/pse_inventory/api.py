"""Buyer-safe public inventory API (packet contract v4.0.0).

Endpoints:
- GET  /api/inventory                       list with pagination, ETag, filters
- GET  /api/inventory/{slug}                detail; unpublished slugs return 404
- POST /api/internal/inventory/revalidate   HMAC-signed, replay-protected
- GET  /internal/healthz, /internal/readyz, /internal/metrics

The API serves only allowlisted public fields from a validated snapshot. It
never receives private-master credentials and never returns stack traces.
"""
...
from ._packet import load_revalidation_module
